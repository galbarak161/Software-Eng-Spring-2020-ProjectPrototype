 ...
 * <ul>
 * <li>The synchronization of the <code>close()</code> method is now limited to
 * the client threads closing sequence. The call to <code>serverClosed()</code>
 * is outside the synchronized block and is preceeded by a join that garantees
 * that <code>serverStopped()</code> is always called before.
 * <li>Method <code>isClosed()</code> has been added.
 * <li>When a client is accepted, the corresponding connection thread will be
 * created only if the server has not been stopped.
 * </ul>
 * The modifications made to this class in version 2.3 are:
 * <ul>
 * <li>An instance variable refering to the current connection factory. Refer to
 * null value by default, in this case regular <code>ConnectionToClient</code>
 * instances are created as in the previous versions.
 * <li>Method <code>setConnectionFactory()</code> has been added.
 * <li>In the run method, a call to the connection factory is made if such a
 * factory is available.
 * <li>Method <code>handleMessageFromClient</code> is not always called
 * depending on the value returned by the <code>handleMessageFromClient</code>
 * of the <code>ConnectionToClient</code> class.
 * <li>The <code>clientException</code> method is still the one called when an
 * exception is thrown when handling the connection with one client. However
 * <code>ClassNotFoundException</code> and <code>RuntimeException</code>
 * instances can now be received.
 * <li>The call to <code>serverStopped()</code> has been moved in the
 * <code>run</code> method.
 * <li>Method <code>isListening()</code> has been modified.
 * <li>Instance variable <code>readToStop</code> is now initialized to
 * <code>true</code>
 * </ul>
 * <p>
 *
...
public abstract class AbstractServer implements Runnable {
	 ...
	 * server will check to see if a command to stop the server has been issued; it
	 * not it will resume accepting connections. Set to half a second by default.
	 */
	...
	/**
	 * The thread group associated with client threads. Each member of the thread
	 * group is a <code> ConnectionToClient </code>.
	 */
	...
	/**
	 * The factory used to create new connections to clients. Is null by default,
	 * meaning that regular <code>ConnectionToClient</code> instances will be
	 * created. Added in version 2.3
	 */
...
// INSTANCE METHODS *************************************************
	...
	/**
	 * Closes the server socket and the connections with all clients. Any exception
	 * thrown while closing a client is ignored. If one wishes to catch these
	 * exceptions, then clients should be individually closed before calling this
	 * method. The method also stops listening if this thread is running. If the
	 * server is already closed, this call has no effect.
	 *
	 ...
	 * messages to all clients. This method can be overriden, but if so it should
	 * still perform the general function of sending to all clients, perhaps after
	 * some kind of filtering is done. Any exception thrown while sending the
	 * message to a particular client is ignored.
	 *
...
// ACCESSING METHODS ------------------------------------------------
	...
	/**
	 * Returns an array containing the existing client connections. This can be used
	 * by concrete subclasses to implement messages that do something with each
	 * connection (e.g. kill it, send a message to it etc.). Remember that after
	 * this array is obtained, some clients in this migth disconnect. New clients
	 * can also connect, these later will not appear in the array.
	 *
	...
	/**
	 * Sets the port number for the next connection. The server must be closed and
	 * restarted for the port change to be in effect.
	 *
	 ...
	 * second. This means that stopping the server may take up to timeout duration
	 * to actually stop. The server must be stopped and restarted for the timeout
	 ...
	 * system. The default is 20. The server must be closed and restarted for the
	 * backlog change to be in effect.
	 *
	...
	/**
	 * Sets the connection factory. Once set, this one will be used in the creation
	 * of new <code>ConnectionToClient</code> instances. The call to this method is
	 * optional; if not called Then regular <code>ConnectionToClient</code>
	 * instances are created. Added in version 2.3
	 *
...
// METHODS DESIGNED TO BE OVERRIDDEN BY CONCRETE SUBCLASSES ---------
	 ...
	 * implementation does nothing. This method does not have to be synchronized
	 * since only one client can be accepted at a time.
	 *
	...
	/**
	 * Hook method called each time a client disconnects. The client is garantee to
	 * be disconnected but the thread is still active until it is asynchronously
	 * removed from the thread group. The default implementation does nothing. The
	 * method may be overridden by subclasses but should remains synchronized.
	 *
	 ...
	 * synchronized. Most exceptions will cause the end of the client's thread
	 * except for <code>ClassNotFoundException<\code>s received when an object of
	 * unknown class is received and for the <code>RuntimeException</code>s that can
	 * be thrown by the message handling method implemented by the user.
	 *
	...
	/**
	 * Hook method called when the server stops accepting connections. The default
	 * implementation does nothing. This method may be overriden by subclasses.
	 */
...
// METHODS TO BE USED FROM WITHIN THE FRAMEWORK ONLY ----------------
	 ...
	 * messages coming from the server This method is synchronized to ensure that
	 * whatever effects it has do not conflict with work being done by other
	 * threads. The method simply calls the <code>handleMessageFromClient</code>
	 * slot method.
	 *
	 * @param msg    the message sent.
	 * @param client the connection connected to the client that sent the message.
